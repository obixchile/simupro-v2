...
<tr>{['TRONCAL','SUBESTACIÓN','REGIÓN','PRECIO BASE','PROPUESTAS','ACCIONES'].map(h=>(<th key={h} style={{padding:'0.75rem 1rem',textAlign:'left',fontSize:'0.7rem',fontWeight:700,color:'#64748b',letterSpacing:'0.05em',whiteSpace:'nowrap'}}>{h}</th>))}</tr>
</thead>
<tbody>
{precios.map(p=>(
<tr key={p.id} style={{borderTop:'1px solid #f1f5f9'}}>
<td style={{padding:'0.875rem 1rem',fontSize:'0.85rem',fontWeight:500,color:'#0f172a',whiteSpace:'nowrap'}}>{p.troncal}</td>
<td style={{padding:'0.875rem 1rem',fontSize:'0.8rem',color:'#475569',whiteSpace:'nowrap'}}>{p.subestacion}</td>
<td style={{padding:'0.875rem 1rem'}}><span style={{background:'#dbeafe',color:'#1d4ed8',padding:'0.2rem 0.5rem',borderRadius:'999px',fontSize:'0.7rem',fontWeight:600,whiteSpace:'nowrap'}}>{p.region}</span></td>
...
<span style={{fontWeight:700,color:'#10b981',fontSize:'0.9rem',whiteSpace:'nowrap'}}>${p.precio.toFixed(2)} USD/MWh</span>
)}
</td>
<td style={{padding:'0.875rem 1rem',fontSize:'0.8rem',color:'#475569',whiteSpace:'nowrap'}}>{p.propuestas} propuestas</td>
<td style={{padding:'0.875rem 1rem'}}>
